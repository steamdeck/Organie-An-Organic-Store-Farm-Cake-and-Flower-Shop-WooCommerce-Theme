 ...
 * Example
	array(
			'type' => 'gradient',
			'class' => '',
			'heading' => esc_html__('Gradient Type', 'tm-organie'),
			'param_name' => 'bg_grad',
			'description' => esc_html__('At least two color points should be selected.', 'tm-organie'),
	...
	class TM_Gradient_Param {
		...
		function gradient_picker( $settings, $value ) {
			...
			$output .= '<select id="grad_type' . $uni . '" class="grad_type" data-uniqid="' . $uni . '">
				<option value="vertical">' . esc_html__( 'Vertical', 'tm-organie' ) . '</option>
				<option value="horizontal">' . esc_html__( 'Horizontal', 'tm-organie' ) . '</option>
				<option value="custom">' . esc_html__( 'Custom', 'tm-organie' ) . '</option>
			</select>
			...
			$output .= '<div class="wpb_element_label" style="margin-top: 10px;">' . esc_html__( 'Choose Colors', 'tm-organie' ) . '</div>';
