...
if ( $max_value && $min_value === $max_value ) {
	...
	?>
	<div class="quantity">
		<span class="qty-minus" data-min="<?php echo esc_attr( $min_value ); ?>">
		...
		<input type="number" id="<?php echo esc_attr( $input_id ); ?>" class="input-text qty text"
		       ...
		       title="<?php echo esc_attr_x( 'Qty', 'Product quantity input tooltip', 'tm-organie' ) ?>" size="4"
		       pattern="<?php echo esc_attr( $pattern ); ?>" inputmode="<?php echo esc_attr( $inputmode ); ?>"
		       aria-labelledby="<?php echo ! empty( $args['product_name'] ) ? sprintf( esc_attr__( '%s quantity', 'tm-organie' ), $args['product_name'] ) : ''; ?>"/>
		<span class="qty-plus" data-max="<?php echo esc_attr( $max_value ); ?>">
